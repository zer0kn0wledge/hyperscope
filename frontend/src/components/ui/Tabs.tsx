...
interface TabsProps {
  ...
  tabs?: TabItem[];        // alias
  activeKey?: string;
  activeTab?: string;      // alias
  onChange?: (key: string) => void;
  onTabChange?: (key: string) => void; // alias
  className?: string;
  size?: 'sm' | 'md';
}

export function Tabs({ items, tabs, activeKey, activeTab, onChange, onTabChange, className = '', size = 'md' }: TabsProps) {
  const resolvedItems = items || tabs || [];
  const resolvedActive = activeKey || activeTab || '';
  ...
  return (
    <div className={`flex gap-0 ${className}`} role="tablist" style={{ borderBottom: '1px solid rgba(0,255,136,0.08)' }}>
      {resolvedItems.map((item) => {
        const active = item.key === resolvedActive;
        return (
          <button
            key={item.key}
            role="tab"
            aria-selected={active}
            onClick={() => resolvedOnChange(item.key)}
